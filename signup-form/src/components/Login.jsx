...
const Login = () => {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    if (result.success) {
      if (result.step === "otp") {
        setStep("otp")
        setTimer(60) // Reset timer when entering OTP screen
        setCanResend(false)
  ...
  const handleResendOtp = async () => {
    setLoading(true)
    const result = await authLogin(email, password) // Login call hi OTP dobara bhej dega
    if (result.success) {
      setTimer(60)
      setCanResend(false)
        ...
        >
          ...
          {step === 'login' ? (
            ...
            <Box component="form" onSubmit={handleOtpSubmit} sx={{ animation: 'fadeInScale 0.5s' }}>
              ...
              <Button type="submit" fullWidth disabled={loading}
                sx={{
                  mt: 1, py: 1.2, borderRadius: 2, fontWeight: 700, textTransform: "none", fontSize: "16px",
                  background: "linear-gradient(90deg, #10b981 0%, #059669 100%)", color: "#fff"
                }}
              >
                {loading ? <CircularProgress size={24} color="inherit" /> : "Verify & Login"}
              </Button>
              <Button fullWidth onClick={() => { setStep('login'); setTimer(0); }} sx={{ mt: 1, textTransform: "none", color: "text.secondary" }}>
                Back to Login
              </Button>
            </Box>
          )}
